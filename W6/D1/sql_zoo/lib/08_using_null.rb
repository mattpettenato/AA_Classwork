...
def teachers_and_divisions
  ...
  execute(<<-SQL)
  SELECT teachers.name, CASE WHEN teachers.dept_id IN (1,2) THEN 'Sci' ELSE 'Art' END
  FROM teachers
  SQL
end
...
def teachers_and_divisions_two
  ...
  # 'None' otherwise.
  execute(<<-SQL)
  SELECT teachers.name, CASE WHEN teachers.dept_id IN (1,2) THEN 'Sci' WHEN teachers.dept_id IN (3) THEN 'Art' ELSE 'None' END
  FROM teachers
  SQL
end
